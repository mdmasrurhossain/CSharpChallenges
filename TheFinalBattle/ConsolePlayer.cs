...
public class ConsolePlayer : IPlayer
{
    ...
    private List<MenuChoice> CreateMenuOptions(Battle battle, Character character)
    {
        ...
        // Add using the Health Potion as an item as an option.
        if (currentParty.Items.Count > 0 && currentParty.Items.Any(x => x is HealthPotion))
            menuChoices.Add(new MenuChoice($"Use Health Potion ({currentParty.Items.Count(x => x is HealthPotion)})", new UseItemAction(currentParty.Items.OfType<HealthPotion>().First())));
        else
            menuChoices.Add(new MenuChoice($"Use Health Potion (0)", null));

        // Add using the Simula's Soup as an item as an option.
        if (currentParty.Items.Count > 0 && currentParty.Items.Any(x => x is SimulasSoup))
            menuChoices.Add(new MenuChoice($"Use Simula's Soup ({currentParty.Items.Count(x => x is SimulasSoup)})", new UseItemAction(currentParty.Items.OfType<SimulasSoup>().First())));
        else
            menuChoices.Add(new MenuChoice($"Use Simula's Soup (0)", null));
