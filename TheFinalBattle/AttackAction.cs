...
public class AttackAction : IAction
{
    ...
    public void Run(Battle battle, Character character)
    {
        ...
        if (character.OffensiveModifier is RockAndStones)
            data = character.OffensiveModifier.Modify(data, _target);

        if (_target.DefensiveModifier != null && character.OffensiveModifier is not RockAndStones)
            data = _target.DefensiveModifier.Modify(data, _target);
